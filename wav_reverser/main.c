...
uint8_t* reverseWav(uint8_t* inputFile, int inputFileSize) {

    //size_t* inputFileSize = getFileSize((char*) inputFile);

    if (inputFileSize == 0)
        return NULL;

    struct WavFile *wavFile = malloc(sizeof(struct WavFile));
    int isValidWavFile = validWavFile((const char*) inputFile, inputFileSize, wavFile);

    if (isValidWavFile != 0) {
        return NULL;
    }

    //Since we forgot, let's leave a note here that dataLoc is the memAddress for the start of the data section for the wavFile.
    //dataAsInts was to make the process of swapping 4-byte groupings together
    ...
    //write_file("output.wav", reversedDataWithHeader, *inputFileSize);
    //freeWavFile(inputFileSize, (char*) &inputFile, wavFile);
    free(reversedDataAsInts);

    return (uint8_t*) reversedDataWithHeader;
